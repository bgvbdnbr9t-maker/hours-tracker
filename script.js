...
function updateUI() {
  ...
  document.getElementById("total").innerText =
    `Total: $${totalPay.toFixed(2)} | ${totalHours} hrs`;
...
function updateChart() {
  ...
  let labels = sorted.map(i => new Date(i.date).toLocaleDateString());
  let data = sorted.map(i => i.pay);
...
function sendReminder() {
  if (Notification.permission === "granted") {
    new Notification("Hours Tracker", {
      body: "Don't forget to log your hours today 💼"
    });
  }
}
